...
def maximum(i,j):
	...
	for k in range(1,4):
		if count[k]>max_value:
			...
			max_value=count[k]
	for k in range(1,4):
		if count[k] == max_value and cell_sum(k,i,j)%2==1:
			max_index =k;
	return max_index	
...
def calc_max_prob():
	...
	for i in range(10):
		for j in range(10):
			if probability[i][j]==max_val and (i+j)%2==1:
				max_index = [i,j]			
...
def hunt():
	txt3=open('no_of_moves.txt','w')
	if all_destroyed()==False:	
		max_index = calc_max_prob()
		txt4.write(str(max_index[0]))
		txt4.write(str(max_index[1]))
		global count_moves
...
def main():
	...
	global txt2,txt4,matrix,ship_matrix,ship_matrix1,ship_size,shipstate,count_moves
	txt2=open('player1.txt')
	txt4 =open('moves.txt','w')
